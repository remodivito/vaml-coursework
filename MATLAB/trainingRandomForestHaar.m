...
% Train a Random Forest model using Haar feature extraction
load('haarcascade_frontalface_alt.mat', 'HaarCascade');
...
% Perform feature extraction on training data using Haar features
train_vectors = [];
for i = 1:size(train_images, 1)
    image = reshape(train_images(i, :), train_original_dimensions(i, 1), []);
    integral_image = GetIntergralImages(image, );
    % Extract Haar features using the HaarCascadeObjectDetection function
    vector = HaarCascadeObjectDetection(integral_image, HaarCascade);
